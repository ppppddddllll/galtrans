...
<p align="center">
  ...
  <br>
  <img src="https://img.shields.io/github/v/release/ppppddddllll/galtrans" alt="Release">
  <img src="https://img.shields.io/github/release-date/ppppddddllll/galtrans" alt="Release date">
  <img src="https://img.shields.io/github/stars/ppppddddllll/galtrans" alt="GitHub stars">
  <img src="https://img.shields.io/github/last-commit/ppppddddllll/galtrans" alt="Last commit">
  <img src="https://img.shields.io/github/repo-size/ppppddddllll/galtrans" alt="Repo size">
  <br>
  <img src="https://img.shields.io/badge/engine-Ren'Py-orange.svg" alt="Ren'Py">
  <img src="https://img.shields.io/badge/engine-Kirikiri%2FKAG-orange.svg" alt="Kirikiri/KAG">
  <img src="https://img.shields.io/badge/OCR-Windows%20OCR-blue.svg" alt="OCR">
  <img src="https://img.shields.io/badge/translate-DeepSeek%2FDeepL%2FGoogle%2FBing-brightgreen.svg" alt="Translate engines">
</p>
...
### 实时翻译（不稳定，网络连接差时延迟非常大；悬浮窗功能正在测试，因此 <font color="red">**不建议使用实时翻译**</font>）
...
实时翻译的「本地模型」引擎可在 **完全离线** 状态下翻译，无网络延迟：

1. 在「设置」页的「本地模型（离线翻译）」分组中，点击「下载模型」（首次约 300MB，需联网一次）。
2. 下载完成后状态显示「✓ 已下载」。国内网络慢时可先在「下载镜像」填入 `https://hf-mirror.com` 再下载。
3. 回到「实时翻译」页，引擎下拉框选择「本地模型（离线最快）」，开始翻译即可全程离线。

> 说明：本地模型为 MarianMT 日中模型（`Helsinki-NLP/opus-mt-ja-zh`），翻译质量属机翻水平、
> 快于联网接口，适合快速理解剧情大意。模型文件存于 `~/.galtrans/models/`，不随程序分发。
...
- `translate.primary` / `translate.fallbacks`：主引擎与降级顺序（`deepseek`/`deepl`/`google`/`bing`/`local`）。
...
- `local.model` / `local.endpoint` / `local.models_dir`：本地模型名、下载镜像、模型目录（默认 `~/.galtrans/models/`）。
...
│   ├── bing.py          #   Bing 免费接口
│   ├── local_model.py   #   本地离线模型（HuggingFace MarianMT）
│   └── manager.py       #   多引擎调度与降级
└── ui/                  # PySide6 图形界面
...
实时翻译是「截屏 → OCR → 联网翻译」循环（默认 0.8 秒一次），延迟由网络与 OCR 耗时决定。追求速度在实时页选 Bing 或 Google；**网络差时推荐使用「本地模型」引擎**——先在设置页下载模型（约 300MB，需联网一次），之后全程离线翻译、无网络延迟。
